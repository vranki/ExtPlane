    ...
    FixedSimulatedDataRef(parent,"",refName)
{
    ...
    for(int i = 0; i < 100; i++) {
        ...
        double localX = (-((int)range*10) + QRandomGenerator::global()->bounded(((int)range*10*2))) / 10.0;
        double localY = (-((int)range*10) + QRandomGenerator::global()->bounded(((int)range*10*2))) / 10.0;
        double lat = 47.4475;
        double lon = 8.0001;
        int typeRand = QRandomGenerator::global()->bounded(5);
        QString type = "1";
        if(typeRand == 0) type = "1";
        if(typeRand == 1) type = "8";
        if(typeRand == 2) type = "512";
        if(typeRand == 3) type = "64";
        int idRand = QRandomGenerator::global()->bounded(5);
        QString id = "----";
        if(idRand == 0) id = "KSEA";
        if(idRand == 1) id = "IBFI";
